...
<section class="panel">
    <div class="table-wrap">
        <table class="data-table" id="clients-table">
            <thead><tr><th>Zařízení / držitel</th><th>IP adresa</th><th>Wi‑Fi síť</th><th>CAP / pásmo</th><th>Signál</th><th>Provoz</th><th>Stav</th></tr></thead>
            <tbody>
            <?php if ($clients === []): ?><tr class="empty-row"><td colspan="7"><span><?= icon('devices') ?></span><strong>Žádná připojená zařízení</strong><small>Data se objeví po první úspěšné synchronizaci.</small></td></tr><?php endif; ?>
            <?php foreach ($clients as $client): ?>
                <?php $clientName = $client['person_name'] ?: ($client['device_name'] ?: ($client['hostname'] ?: 'Neznámé zařízení')); $signal = isset($client['signal_dbm']) ? (int)$client['signal_dbm'] : null; $status = $client['registration_status']; ?>
                <tr data-search-row>
                    ...
                    <td><strong><?= e($client['rx_rate'] ?: '—') ?></strong><small class="subline">↓ <?= number_format(((int)$client['rx_bps'])/1000000, 1, ',', ' ') ?> Mb/s</small></td>
