...
final class UsersController
{
    ...
    public function index(): void
    {
        ...
        $users = $this->database->pdo()->query('SELECT id, username, display_name, role, active, last_login_at, created_at FROM admin_users ORDER BY active DESC, display_name')->fetchAll();
    ...
    public function store(): void
    {
        ...
        $statement = $this->database->pdo()->prepare(
            'INSERT INTO admin_users (username, username_normalized, display_name, password_hash, role) VALUES (:username, :normalized, :display_name, :password_hash, :role)'
        );
        $statement->execute([
            'username' => $username, 'normalized' => mb_strtolower($username), 'display_name' => $displayName,
            'password_hash' => password_hash($password, password_algorithm()), 'role' => $role,
        ]);
